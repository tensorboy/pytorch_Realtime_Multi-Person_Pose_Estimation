...
if __name__ == '__main__':
    ...
    rootdir = os.path.join(DATA_DIR, cfg.DATASET.TRAIN_IMAGE_DIR)
    annfiles = os.path.join(SOURCE_DIR, cfg.DATASET.TRAIN_ANNOTATIONS[0])

    # train_datas = [datasets.CocoKeypoints(
    #     root=cfg.DATASET.TRAIN_IMAGE_DIR,
    #     annFile=os.path.join(SOURCE_DIR, item),
    ...
    val_loader = torch.utils.data.DataLoader(
        val_data, batch_size=18, shuffle=False,
        pin_memory=cfg.PIN_MEMORY, num_workers=cfg.WORKERS, drop_last=True)

    vis_dir = os.path.join(SOURCE_DIR, 'data/coco/images/vis_val2017(2)')
    print(vis_dir)
    if not os.path.exists(vis_dir):
        os.makedirs(vis_dir)

    for batch_idx, (image, heatmaps, pafs) in enumerate(val_loader):
        print('batch_idx:', batch_idx)
        print('image.shape:', image.shape)
        print('heatmaps.shape:', heatmaps.shape)
        print('pafs.shape:', pafs.shape)
        index = np.random.randint(0, 18)        # pick one image per batch randomly
        ...
        joint_list, person_to_joint_assoc = paf_to_pose(heatmap, paf, cfg)
        ...
        for keypoint in keypoints:
            for j in range(8):
                joint = keypoint[j]
                if joint[2] == 1:
                    cv2.circle(img, tuple(joint[0:2].astype(int)), 2, colors[j], thickness=2)
